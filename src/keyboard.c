...
/* 87.6.6   modified by K.Handa */
/* 87.6.13  modified by K.Handa */
/* 87.12.19 modified for Nemacs Ver.2.0 by K.Handa */
/* 88.4.27  modified by Y.Ishikawa */
/* 88.5.26  modified for Nemacs Ver.2.1 by K.Handa */
/* 89.2.10  modified for Nemacs Ver.3.0 by K.Handa */
/* 89.12.20 modified for Nemacs Ver.3.2.3 by K.Handa
	Handle C-g correctly when kanji-input-code is Shift-JIS */
...
/* 92.3.23  modified for Mule Ver.0.9.1 by K.Handa <handa@etl.go.jp>
	In Fexecute_extended_command(), set this_command_keys correctly. */
...
/* 93.7.8   modified for Mule Ver.0.9.8 by T.Atsushiba <toshiki@jit.dec.co.jp>
	Function type declaration of recursive_edit_1() and
	Fthis_command_keys(). */
...
/* Nonzero if should mask 0200 bit as parity-bit */
int parity_flag;		/* 92.1.28 by K.Handa */

extern char *pending_malloc_warning;

/* 91.12.17 by M.Higashida */
#ifdef MSDOS
int ibmpc_enhanced_keyboard;
#endif
/* end of patch */
...
command_loop_1 ()
{
  ...
  while (1)
	...
	return Qnil;		/* a kbd macro, at the end */
      ...
      last_command_char = 	/* 91.11.19 by K.Handa */
	(unsigned char)keybuf[i - 1];
      ...
      else
	{
	  ...
	  if (NULL (Vprefix_arg) && ! no_direct)
	      ...
	      if (EQ (cmd, Qforward_char) && point < ZV)
		{
		  ...
		  SET_PT (forward_point(1)); /* 91.10.29 by K.Handa */
		  if (lose >= ' ' && lose < 0177
	      ...
	      else if (EQ (cmd, Qself_insert_command))
		{
		  if (NULL (Vexecuting_macro) &&
		    ...
		    {
		      if (!DisableUndo && /* by Y.Ishikawa */
			  (!nonundocount || nonundocount >= 20))
			{
			  Fundo_boundary ();
			  nonundocount = 0;
			}
	  ...
	  nonundocount = 0;
	  if (!DisableUndo && NULL (Vprefix_arg)) /* by Y.Ishikawa */
	    Fundo_boundary ();
...
/* 91.10.20, 93.2.17 by M.Higashida */
      ...
      time (&tnow);
      tthen = tnow + echo_keystrokes;
      echo_flag = 0;
      echo_now = 0;
      while (!input_pending && !detect_input_pending ())
	{
	  time (&tnow);
	  if (tnow >= tthen)
	    break;
...
/* end of patch */
  ...
  c &= parity_flag ? 0177 : 0377; /* 91.11.19 by K.Handa */
     ...
     register int c;
{
  ...
  if (c == quit_char		/* 91.11.19 by K.Handa */
      || ((c == (0200 | quit_char)) && parity_flag))
    { 
      interrupt_signal ();
      return;
    }
...
/* Note SIGIO has been undef'd if FIONREAD is missing.  */
  ...
  /* 91.11.19 by K.Handa */
  /* Clean up if sign was extended. */
  return (c & (parity_flag ? 0177 : 0377));
}

...
#ifdef HAVE_THREAD
#if 1
      *addr = WaitForSingleObject (hWaitObject[0], 0);
#else
      *addr = PeekMessage (&msg, (HWND) 0, 0, 0, PM_NOREMOVE);
#endif
#else /* not HAVE_THREAD */
      *addr = PeekMessage (&msg, hwndMain, 0, 0, PM_NOREMOVE);
#endif /* not HAVE_THREAD */
      if (*addr == 0 || read_socket_hook == 0) 
        return; /* Nothing but read from `read_avail_input'. */
    }
#else /* not WIN32 */
#ifdef FIONREAD
  if (! force_input)
...
#if defined(FIONREAD) || defined(WIN32) /* 93.2.17 by M.Higashida */
  if (! force_input)
    {
      if (nread == 0)
	get_input_pending (&nread);
     ...
     int nodisplay;
{
  ...
  Lisp_Object nextfront, front;
/* 91.11.29, 93.3.3 by M.Higashida */
  ...
  Lisp_Object FEPstat = Ffep_get_mode ();
#endif
/* end of patch */
  ...
  nextfront = current_buffer->frontmap;
  nextlocal = current_buffer->keymap;
  XSET (nextglobal, Lisp_Vector, global_map);

  i = 0;
  while (!NULL (nextfront) || !NULL (nextlocal) || !NULL (nextglobal))
    {
      if (i == bufsize)
	...
	c = read_command_char (!prompt);
      ...
      if (c < 0)
	return 0;
      if (c >= 0200 && meta_key) /* 91.10.29 by K.Handa */
	{
	  nextc = c & 0177;
	  c = meta_prefix_char;
	}
	...
	? get_keyelt (access_keymap (nextlocal, c))
      ...
      front = !NULL (nextfront)
	? get_keyelt (access_keymap (nextfront, c))
	  : Qnil;

      /* If C is not defined in either keymap
	 and it is an uppercase letter, try corresponding lowercase.  */
      /* ... but not with front map. */

      if (NULL (front) && NULL (global) && NULL (local) && UPPERCASEP (c))
	{
	  global = !NULL (nextglobal)
	...
	!NULL (front) ? front : !NULL (local) ? local : global;
	...
	local = XSYMBOL (local)->function;
      while (XTYPE (front) == Lisp_Symbol && !NULL (front)
	     && !EQ (front, Qunbound))
	front = XSYMBOL (front)->function;
	  ...
	  ||
	  XTYPE (front) == Lisp_Vector ||
	  (CONSP (front) && EQ (XCONS (front)->car, Qkeymap))
	  ||
...
/* 91.11.29, 93.3.3 by M.Higashida */
#if (defined(MSDOS) && defined(FEPCTRL)) || (defined(WIN32) && defined(USE_IME))
          if (!NULL (FEPstat)) Ffep_force_off ();
#endif
/* end of patch */
	}
    }
...
#ifndef WIN32 /* 93.2.25 by M.Higashida */
  ...
  if (!NULL (quit))
    {
      ...
      quit_char &= parity_flag ? 0177 : 0377;
    }
  init_sys_modes ();
  return Qnil;
}
...
init_keyboard ()
{
  ...
  input_pending = 0;
  force_input = 0;
  parity_flag = 0;		/* 91.11.19 by K.Handa */
  if (!noninteractive)
    {
      signal (SIGINT, interrupt_signal);
#ifdef HAVE_TERMIO
...
\(has a non-nil  disabled  property).");
 ...
 /* 91.11.19 by K.Handa */
  DEFVAR_BOOL ("parity-flag", &parity_flag,
    "*Non-nil means treat 0200 bit as parity-bit and mask it.");
...
/* by Y.Ishikawa */
  DEFVAR_BOOL ("disable-undo", &DisableUndo,
    "*Nil means undo facility is active.\n\
This feature is only available in Mule.");
  DisableUndo = 0;
/* end of patch */

/* 91.12.17 by M.Higashida */
#ifdef MSDOS
  DEFVAR_BOOL ("ibmpc-enhanced-keyboard", &ibmpc_enhanced_keyboard, "");
  ibmpc_enhanced_keyboard = 0;
#endif
/* end of patch */
}
