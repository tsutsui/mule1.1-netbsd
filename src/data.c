...
Lisp_Object Qcharacterp, Qmnemonicp; /* 91.11.17 by K.Handa */
...
DEFUN ("setcar", Fsetcar, Ssetcar, 2, 2, 0,
  ...
  (register Lisp_Object cell, register Lisp_Object newcar)
{
...
DEFUN ("setcdr", Fsetcdr, Ssetcdr, 2, 2, 0,
  ...
  (register Lisp_Object cell, register Lisp_Object newcdr)
{
  if (XTYPE (cell) != Lisp_Cons)
    cell = wrong_type_argument (Qconsp, cell);
...
DEFUN ("fset", Ffset, Sfset, 2, 2, 0,
  ...
  (register Lisp_Object sym, register Lisp_Object newdef)
{
  CHECK_SYMBOL (sym, 0);
...
DEFUN ("setplist", Fsetplist, Ssetplist, 2, 2, 0,
  ...
  (register Lisp_Object sym, register Lisp_Object newplist)
{
  CHECK_SYMBOL (sym, 0);
...
Lisp_Object
do_symval_forwarding (register Lisp_Object valcontents)
{
  register Lisp_Object val;
...
void
store_symval_forwarding (Lisp_Object sym, register Lisp_Object valcontents, register Lisp_Object newval)
{
#ifdef SWITCH_ENUM_BUG
  switch ((int) XTYPE (valcontents))
#else
  ...
  (register Lisp_Object sym, register Lisp_Object newval)
{
  ...
  if (XTYPE (valcontents) == Lisp_Buffer_Objfwd)
    {
      register int idx = XUINT (valcontents);
      register Lisp_Object_Int mask = *(Lisp_Object_Int *)(idx + (char *) &buffer_local_flags);
      if (mask > 0)
...
syms_of_data (void)
{
  ...
  Qcharacterp = intern ("characterp"); /* 91.11.17 by K.Handa */
  Qmnemonicp = intern ("mnemonicp"); /* 91.12.5 by K.Handa */
  ...
  staticpro (&Qsymbolp);
  staticpro (&Qintegerp);
  staticpro (&Qcharacterp);	/* 91.11.17 by K.Handa */
  staticpro (&Qmnemonicp);	/* 91.12.6 by K.Handa */
  staticpro (&Qnatnump);
  staticpro (&Qstringp);
  staticpro (&Qarrayp);
