...
;;; 92.3.31  modified for Mule Ver.0.9.2
;;;	by M.Kuwada <kuwada@soliton.ee.uec.ac.jp>
;;;	byte-compile-file should write out file without code-conversion.
;;; 92.7.2  modified for Mule Ver.0.9.5 by K.Handa <handa@etl.go.jp>
;;;	byte-compile-file calls si:write-region instead of write-region.
;;; 93.3.15  modified for Mule Ver.0.9.7.1 by K.Handa <handa@etl.go.jp>
;;;	byte-compile-file calls write-region instead of si:write-region.
;;; 93.7.14  modified for Mule Ver.0.9.8 by K.Handa <handa@etl.go.jp>
;;;	In byte-compile-file-form, special top level form is handled.
...
The output file's name is made by appending \"c\" to the end of FILENAME."
  ...
  (let ((inbuffer (get-buffer-create " *Compiler Input*"))
    ...
    (save-excursion
      (set-buffer inbuffer)
      (erase-buffer)
      ;; 92.11.24 by M.Higashida
      (if (boundp 'file-translation-mode)
	  (setq file-translation-mode *dos-text*))
      ;; end of patch
      ...
      ;; 92.11.24 by M.Higashida
      (if (boundp 'file-translation-mode)
	  (setq file-translation-mode nil))
      ;; end of patch
	...
	(write-region 1 (point-max)	; 92.7.2, 93.3.15 by K.Handa
			 (concat (file-name-sans-versions filename) "c")
			 nil nil *noconv*;; 92.3.31 by M.Kuwada
			 ))
...
(defun byte-compile-file-form (form)
  (cond ((not (listp form))
	...
	((eq (car form) 'require)
	 (eval form)
	 form)
	((get (car form) 'byte-hunk-handler) ;93.7.14 by K.Handa, 7.16 S.Tomura
	 (funcall (get (car form) 'byte-hunk-handler) form))
	(t form)))

