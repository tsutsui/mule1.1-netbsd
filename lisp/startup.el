...
;;; 91.11.7  modified for Nemacs Ver.4.0.0 by K.Handa <handa@etl.go.jp>
...
;;;	'command-line' refers variable user-init-file.
;;; 92.12.16 modified for Mule Ver.0.9.7 by W.S.Rupprecht <wolfgang@wsrcc.COM>
;;;	In normal-top-level, treatment of an shell variable PWD was changed.
...
(defun normal-top-level ()
  (if command-line-processed
    ...
    (if (and (not (eq system-type 'vax-vms))
	     (getenv "PWD")
	     ;; 92.12.16 by W.S.Rupprecht
	     (equal (file-attributes (getenv "PWD"))
  ...
  (let ((args (cdr command-line-args))
	...
	(if init
	    (progn (load (if (eq system-type 'vax-vms)
			     "sys$login:.emacs"
			   ;; 92.10.16 by K.Handa
			     (concat "~" init "/" user-init-filename))
			 t t t)
...
;;; patch by K.Handa  88.1.4, 92.3.5
		   (insert "\n\n"
			   (mule-version))
		   (insert (substitute-command-keys
			    "
Type \\[help-with-tutorial-for-mule] for a Japanese tutorial on using Mule.
For any other Mule specific information,
please read "))
		   (insert exec-directory "FAQ.Mule.")
;;; end of patch
