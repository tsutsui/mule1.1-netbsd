...
/* Modified for vms-pp.com.
 * Define MULE if you use vms-pp.com. 
 *	06-MAR-1993	T.Atsushiba
 */
#define MULE
...
#define False 0
#endif
static int long_name_modified = False;  /* True if long name was modified. */
...
#else
  if((argc != 3) && (argc != 2)) {		/* check argument count */
    fprintf(stderr,"usage: vms_pp infile [outfile]");
#endif /* MULE */
    exit();
  }
 ...
 */
  ...
  if (argc == 3) {
    ...
    if((out = fopen(argv[1],"w")) == NULL) {
      fprintf(stderr,"vms_pp: unable to create file '%s'",argv[1]);
      exit();
    }
#endif /* MULE */
  }
...
#ifdef MULE
  if (long_name_modified == True)
    fprintf(stderr,"VMS_PP Changed: %s\n", argv[1]);
#endif /* MULE */
}
