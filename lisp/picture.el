...
;;; 91.11.22 modified for Nemacs Ver.4.0.0 by K.Handa <handa@etl.go.jp>
...
;;;	Bug of horizontal left move in picture-insert-string fixed.
;;; 92.9.8   modified for Mule Ver.0.9.6 by T.Shingu <shingu@cpr.canon.co.jp>
;;;	picture-insert-string does nothing if string is NULL.
;;; 93.9.9   modified for Mule Ver.1.1 by H.Yasuda <ysd@sm.sony.co.jp>
;;;	Bug of picture-clear-column on multi-byte-character fixed.
;;; 93.9.10  modified for Mule Ver.1.1 by K.Handa <handa@etl.go.jp>
;;;	Bug of picture-backward-clear-column on multi-byte-character fixed.
;;; 93.9.13  modified for Mule Ver.1.1 by H.Yasuda <ysd@sm.sony.co.jp>
;;;	Bug of picture-insert-rectangle fixed.

(provide 'picture)

;; 89.9.7, 91.11.25 by K.Handa
(defun move-to-column-force (column &optional force)
...
If optional arg FORCE is non nil and the goal column is occupied by
 multi-column character, the character is replaced by spaces
 to locate the cursor at the exact column."
  (move-to-column column)
  (let ((col (current-column)))
    (if (< col column)
	(indent-to column)
      (if (and (/= col column) (>= column 0) 		; 93.9.9 by H.Yasuda
	       (or force (= (preceding-char) ?\t)))	; 93.9.9 by H.Yasuda
...
(defun picture-forward-column (arg &optional force) ; 89.9.7  by K.Handa
  "Move cursor right, making whitespace if necessary.
With argument, move that many columns."
  (interactive "p")
  (move-to-column-force (+ (current-column) arg) force))

(defun picture-backward-column (arg)
  "Move cursor left, making whitespace if necessary.
With argument, move that many columns."
  (interactive "p")
  (let ((clm (current-column)))		; 92.3.13 by K.Handa
    (move-to-column-force (- (current-column) arg))
    (if (= clm (current-column))
	(forward-char -1))))

;; 93.9.13 by K.Handa
(defun picture-move-down (arg &optional force)
  "Move vertically down, making whitespace if necessary.
With argument, move that many lines."
  (interactive "p")
  (let ((col (current-column)))
    (picture-newline arg)
    (move-to-column-force col force)))
...
;;; 89.9.7  by K.Handa
(defun picture-movement-wnw ()
  "Move up and left and left after self-inserting character in Picture mode."
  (interactive)
  (picture-set-motion -1 -2))

(defun picture-movement-ene ()
  "Move up and right and right after self-inserting character in Picture mode."
  (interactive)
  (picture-set-motion -1 2))

(defun picture-movement-wsw ()
  "Move down and left and left after self-inserting character in Picture mode."
  (interactive)
  (picture-set-motion 1 -2))

(defun picture-movement-ese ()
  "Move down and right and right after self-inserting character in Picture mode."
  (interactive)
  (picture-set-motion 1 2))
;;; end of patch
...
The mode line is updated to reflect the current direction."
  ...
  (setq mode-name
	(format "Picture:%s"
		(car (nthcdr (+ 2 (% horiz 3) (* 5 (1+ (% vert 2))))
			     '(wnw nw up ne ene Left left none right Right
				   wsw sw down se ese)))))  ; 89.9.7 by K.Handa
  ;; Kludge - force the mode line to be updated.  Is there a better
  ;; way to this?
  (set-buffer-modified-p (buffer-modified-p))
  (message ""))

(defun picture-move (&optional force)	; 89.9.7 by K.Handa
  "Move in direction of  picture-vertical-step  and  picture-horizontal-step."
  (picture-move-down picture-vertical-step force) ;93.9.13 by K.Handa
  (picture-forward-column picture-horizontal-step force))
...
Two arguments BEGIN and END point the head and tail of just inserted string."
  (let ((str (buffer-substring begin end)))
    ...
    (picture-insert-string str)))

(defun picture-insert-string (str)	; 91.11.25, 92.7.10 by K.Handa
  "Insert characters in STR one by one in place of character previously
...
Do \\[command-apropos]  picture-movement  to see those commands."
  ...
  (let  ((len (length str)))
	...
	(let* ((idx 0)
	       ...
	       (width (char-width c))
	       clm)
	  (while idx				; Set to nil at the end of loop
	    (setq clm (current-column))
	    (delete-text-in-column clm (+ clm width))
	    (insert c)
	    (forward-char -1)
	    (setq idx (+ idx (char-bytes c)))
	    (if (< idx len)
		(setq c (sref str idx) width (char-width c))
	      (setq idx nil))
	    (if (/= picture-vertical-step 0)
		(picture-move 'force)
	      (if (> picture-horizontal-step 0)
		  (forward-char 1)
		(move-to-column-force (- clm width) 'force)))
	    )))))
;;; end of patch
...
(defun picture-clear-column (arg)
  ...
  (let* ((opoint (point))
    ...
    (move-to-column-force target-col t)	; 93.9.9 by H.Yasuda
...
(defun picture-backward-clear-column (arg)
  ...
  (interactive "p")
  (move-to-column-force (- (current-column) arg) t) ;93.9.10 by K.Handa
  ...
  (let ((indent-tabs-mode nil))
	...
	(save-excursion
	  (delete-rectangle (point)
			    (progn
			      (picture-forward-column ;93.9.13 by H.Yasuda
			       (string-width (car rectangle)) t)
			      (picture-move-down (1- (length rectangle)) t)
			      (point)))))
    (push-mark)
...
(if (not picture-mode-map)
    (let ((i ?\ ))
      ...
      (if (boundp 'EGG)			; 89.9.7 by K.Handa
	  (while (< i ?\177)
	    (aset picture-mode-map i 'egg-self-insert-command)
	    (setq i (1+ i)))
	(while (< i ?\177)
      ...
      ;;; 89.9.7 by K.Handa
      (define-key picture-mode-map "\C-ck" 'picture-insert-string)
...
they are not defaultly assigned to keys."
  ...
  (if (eq major-mode 'edit-picture)
    ...
    ;;; 89.9.17  patch by K.Handa
    (make-local-variable 'picutre-old-self-insert-after-hook)
    (setq picutre-old-self-insert-after-hook self-insert-after-hook)
    (setq self-insert-after-hook 'picture-insert-after-hook)
    ;;; end of patch
    (picture-set-motion 0 1)
...
With no argument strips whitespace from end of every line in Picture buffer
  ...
  (if (not (eq major-mode 'edit-picture))
    ...
    ;;; 89.9.17  patch by K.Handa
    (setq self-insert-after-hook picutre-old-self-insert-after-hook)
    ;;; end of patch
