...
/* 91.10.19 modified for Nemacs Ver.4.0.0 by K.Handa <handa@etl.go.jp> */
/* 92.3.6   modified for Mule Ver.0.9.0 by K.Handa <handa@etl.go.jp> */
/* 92.4.8   modified for Mule Ver.0.9.3 by T.Enami <enami@sys.ptg.sony.co.jp>
	In read_minibuf(), 'initial' and 'prompt' are also gc-protected. */
/* 92.9.20  modified for Mule Ver.0.9.6 by T.Enami <enami@sys.ptg.sony.co.jp>
	Support non Ascii completion. */
...
Lisp_Object Vminibuf_preprompt; /* 92.2.17 by K.Handa */
     ...
     int expflag;
{
  ...
  struct gcpro gcpro1, gcpro2, gcpro3, gcpro4; /* 92.4.8 by T.Enami */
  ...
  GCPRO4 (minibuf_save_vector[minibuf_level].help_form,
	  ...
	  initial, prompt);	/* 92.4.8 by T.Enami */
     ...
     Lisp_Object prompt, initial_input, keymap, read;
{
  ...
  return read_minibuf (keymap, initial_input, prompt, !NILP(read));
}
     ...
     int len;
{
  ...
  if (NONASCII_P (*s1)) return 0; /* 92.11.17 by K.Handa */
  ...
  if (l == 0)
   ...
   {			/* 92.9.20, 93.1.13 by T.Enami, 92.11.17 by K.Handa */
     int match = len - l;
     while (NONASCII_P (*--s1)) match--; /* termination of the loop assured. */
     return match;
   }
}

  ...
  ()
{
  ...
  for (; i < XSTRING (completion)->size; i++)
    if (ASCII_P (completion_string[i]) /* 92.11.16 by K.Handa */
	&& SYNTAX (completion_string[i]) != Sword) break;
  if (i < XSTRING (completion)->size)
    i = i + 1;

#if 0
/* 92.9.20 by T.Enami, 92.11.17 by K.Handa */
  while (i > 0 && NONASCII_P(completion_string[i]))
    i--;
  if (LC_P(completion_string[i]))
    i += completion_string[i] == LCCMP /* 92.11.12 by K.Handa */
      ? char_bytes(completion_string[i + 1] - 0x20)
      : char_bytes[completion_string[i]];
/* end of patch */
#endif
     ...
     Lisp_Object str;
{
  ...
  minibuf_prompt
    = (char *) xrealloc (minibuf_prompt,
			 minibuf_prompt_width + XSTRING (str)->size + 1);
  bcopy (XSTRING (str)->data, minibuf_prompt + minibuf_prompt_width,
	 XSTRING (str)->size + 1);
  windows_or_buffers_changed++;
}
...
/* 92.2.17 by K.Handa */
  DEFVAR_LISP ("minibuffer-preprompt", &Vminibuf_preprompt,
    "Non nil means pre-prompt to be displayed before minibuf-prompt.");
...
/* end of patch */
  ...
  defsubr (&Sminibuffer_extend_prompt);
}
