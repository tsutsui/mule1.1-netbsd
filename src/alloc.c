...
/* 92.7.30  modified for Mule Ver.0.9.5
	...
	CANNA requires bigger NSTATICS. */
/* 92.10.12 modified for Mule Ver.0.9.6 by K.Handa <handa@etl.go.jp>
	EGG also requires bigger NSTATICS now. */
  ...
  ()
{
  ...
  p->bufpos = 0;
  p->chain = Qnil;
  p->type = MARKER_OLD_TYPE;	/* 89.11.30  by K.Handa */
...
{
  register Lisp_Object val;
  val = make_uninit_string (length); /* 93.7.8 by T.Atsushiba */
...
/* 92.7.30 by I.Hirakura, 92.10.12, 93.6.18 by K.Handa */
#define NSTATICS 220
/* end of patch */
