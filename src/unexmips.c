...
/* This over comes the problem of lisp.h defining NULL */
#ifdef DECOSF
#undef NULL
#define NULL 0L
#endif
     ...
     unsigned Lisp_Object_Int data_start, bss_start, entry_address;
{
  ...
  Lisp_Object_Int newsyms, symrel;
  int nread;
  struct headers hdr;
  int i;
  Lisp_Object_Int vaddr, scnptr;
#define BUFSIZE 8192
  char buffer[BUFSIZE];
  ...
  if (entry_address == 0)
    {
      ...
      hdr.aout.entry = (unsigned Lisp_Object_Int)DEFAULT_ENTRY_ADDRESS;
    }
  else
