...
;;;	Modified for a system which does not have subprocesses.
;;;	'grep' uses variable 'null-filename'.

(provide 'compile)

;; 92.10.16 by K.Handa
(if (and (fboundp 'processp)
	 (not (eq system-type 'win32)))
    nil
  (defun start-process (procname buffername &rest ignore)
    "Dummy function."
    buffername)
  (defun set-process-sentinel (&rest ignore)
    "Dummy function.")
  (defun process-buffer (buffername)
    "Dummy Function."
    (get-buffer buffername)) ; 92.12.16 by K.Masuda <masuda@kaba.or.jp>
  (defun call-compile-process ()
    (message "Running program...")
    (goto-char (point-max))
    (call-process shell-file-name nil
		  standard-output nil shell-command-option command)
    (setq compilation-process nil) ; 92.12.16 by K.Masuda <masuda@kaba.or.jp>
    (message "Running program...done"))
  (defun process-status (a)) ; 93.3.4 by M.Higashida
  (defvar compile-hook 'call-compile-process
    "A hook called at the tail of function 'compile1'.")
)
;; end of patch
...
to find the text that grep hits refer to."
  ...
  (compile1 (concat "grep -n " command " " null-filename) ;92.10.16 by K.Handa
...
(defun compile1 (command error-message &optional name-of-mode)
  ...
  (let* ((thisdir default-directory)
	...
	(or (eq outwin (selected-window))
      ...
      (run-hooks 'compile-hook)		;92.10.16 by K.Handa
      )))
