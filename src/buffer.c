...
/* 92.4.10  modified for Mule Ver.0.9.3 by K.Handa <handa@etl.go.jp>
	In Ferase_buffer(), attributed-region set to nil. */
...
/* 92.10.22 modified for Mule Ver.0.9.6 by K.Handa <handa@etl.go.jp>
	validate_position() should not set pos out of range. */
/* 92.10.30 modified for Mule Ver.0.9.6 by T.Enami <enami@sys.ptg.sony.co.jp>
	validate_position() should check (pos < ZV) always. */
/* 92.11.16 modified for Mule Ver.0.9.6 by K.Handa <handa@etl.go.jp>
	Fget_buffer_create() allocates an extra byte for an anchor. */
/* 92.12.17 modified for Mule Ver.0.9.7 by Y.Niibe <gniibe@mri.co.jp>
        Preliminary support (92.8.2) for right-to-left languages. */
/* 93.2.17  modified for Mule Ver.0.9.7 
		by M.Higashida <manabu@sigmath.osaka-u.ac.jp>
	File translation mode supported. */
...
/* 93.3.3   modified for Mule Ver.0.9.7
		by M.Higashida <manabu@sigmath.osaka-u.ac.jp>
	Win32 IME API supported. */
/* 93.7.7   modified for Mule Ver.0.9.8 by K.Handa <handa@etl.go.jp>
	confliction of display_direction and file_translation_mode resolved. */
...
/* 92.11.1, 93.2.25 by M.Higashida */
  ...
  (register Lisp_Object name)
{
  ...
  if (!NILP (buf)) return buf;

  b = (struct buffer *) malloc (sizeof (struct buffer));
  if (!b) memory_full ();

  BUF_GAP_SIZE (b) = 20;
  /* 92.11.16 by K.Handa -- add an extra byte for an anchor */
  data = (unsigned char *) malloc (BUF_GAP_SIZE (b) + 1);
  if (!data)
    {
      free (b);
      memory_full ();
    }
  data[BUF_GAP_SIZE (b)] = 0;	/* 92.11.16 by K.Handa */
  BUF_BEG_ADDR (b) = data;
  BUF_PT (b) = 1;
  BUF_GPT (b) = 1;
  BUF_BEGV (b) = 1;
...
reset_buffer (register struct buffer *b)
{
  ...
  b->read_only = Qnil;
/* 89.11.21, 92.1.30 patch for attribute of characters by K.Handa */
  b->attributed_region = Qnil;
/* end of patch */
...
set_buffer_internal (register struct buffer *b)
{
  ...
  if (XWINDOW (selected_window) != 0)
    swb = NILP (selected_window) ? 0 : XBUFFER (XWINDOW (selected_window)->buffer);
#endif
  ...
  (void)
{
  ...
  current_buffer->attributed_region = Qnil; /* 92.4.10 by K.Handa */
...
/* 91.11.14, 92.10.22 by K.Handa */
  if (!NILP (current_buffer->mc_flag)) {
    XFASTINT (*b) =
      validate_position (XFASTINT (*b), 0);
    XFASTINT (*e) =
      validate_position (XFASTINT (*e), 1);
  }
/* end of patch */
}
...
/* 91.11.14, 92.10.22 by K.Handa */
int
validate_position (register int pos, register int forward)
{
  if ((pos < ZV) && NONASCII_P (FETCH_CHAR (pos))) { /* 92.10.30 by T.Enami */
    if (forward) INC_POS(pos);
    else if (BEGV < pos) DEC_POS(pos);
  }
  return pos;
}
/* end of patch */
  ...
  (void)
{
  ...
  for (alist = current_buffer->local_var_alist; !NILP (alist); alist = XCONS (alist)->cdr)
    {
...
init_buffer_once (void)
{
  ...
  buffer_defaults.ctl_hexa = Qnil; /* 92.4.9 by K.Handa */
  buffer_defaults.mc_flag = Qt;	/* 91.10.27 by K.Handa */
  buffer_defaults.file_coding_system = Qnil; /* 91.10.27 by K.Handa */
  buffer_defaults.display_direction = Qnil; /* 92.8.2 by Y.Niibe */

/* 91.10.16 by S.Hirano, 91.12.6, 92.11.1, 93.3.3 by M.Higashida */
#ifdef FILE_TRANSLATION_MODE
  buffer_defaults.file_translation_mode = Qnil;
#endif
#if (defined(MSDOS) && defined(FEPCTRL)) || (defined(WIN32) && defined(USE_IME))
  buffer_defaults.fep_mode = Qnil;
#endif
/* end of patch */
...
/* 89.11.21 patch for attribute of characters by K.Handa */
  XFASTINT (buffer_local_flags.attributed_region) = -1;
/* end of patch */
...
/* 91.10.27 by K.Handa */
  XFASTINT (buffer_local_flags.mc_flag) = 0x4000;
  XFASTINT (buffer_local_flags.file_coding_system) = 0x8000;
  XFASTINT (buffer_local_flags.category_table) = 0x10000;
  XFASTINT (buffer_local_flags.ctl_hexa) = 0x20000; /* 92.4.9 by K.Handa */
/* end of patch */
/* 92.8.2 by Y.Niibe */
  XFASTINT (buffer_local_flags.display_direction) = 0x40000;
/* end of patch */
/* 91.10.16 by S.Hirano, 91.12.6, 92.11.1, 93.3.3 by M.Higashida */
#if (defined(MSDOS) && defined(FEPCTRL)) || (defined(WIN32) && defined(USE_IME))
  XFASTINT (buffer_local_flags.fep_mode) = 0x80000;
#endif
#ifdef FILE_TRANSLATION_MODE
  XFASTINT (buffer_local_flags.file_translation_mode) = 0x100000;
#endif
/* end of patch */
...
/* 92.4.9 by K.Handa */
  DEFVAR_LISP_NOPRO ("default-ctl-hexa",
	      &buffer_defaults.ctl_hexa,
    "Default ctl-hexa for buffers that do not override it.\n\
This is the same as (default-value 'ctl-hexa).");
/* end of patch */
/* 92.8.2 by Y.Niibe */
  DEFVAR_LISP_NOPRO ("default-display-direction",
	      &buffer_defaults.display_direction,
...
#ifdef FILE_TRANSLATION_MODE
  DEFVAR_LISP_NOPRO ("default-file-translation-mode",
		     &buffer_defaults.file_translation_mode,
    "Default file-translation-mode for buffers that do not override it.\n\
This is the same as (default-value 'file-translation-mode).");
#endif
/* end of patch */

/* 87.12.9, 91.10.27  by K.Handa */
  DEFVAR_LISP_NOPRO ("default-mc-flag",
	      &buffer_defaults.mc_flag,
    "Default mc-flag for buffers that do not override it.\n\
This is the same as (default-value 'mc-flag).");

  DEFVAR_LISP_NOPRO ("default-file-coding-system",
              &buffer_defaults.file_coding_system,
    "Default file-coding-system for buffers that do not override it.\n\
...
/* 92.4.9 by K.Handa */
  DEFVAR_PER_BUFFER ("ctl-hexa", &current_buffer->ctl_hexa,
    "*Non-nil means display control chars with hexadecimal (e.g. \\xFF).\n\
Nil means use backslash and octal digits.\n\
As for chars less than 0x20, ctl-arrow overided this flag.\n\
Automatically becomes local when set in any fashion.");
/* end of patch */
/* 92.8.2 by Y.Niibe */
  DEFVAR_PER_BUFFER ("display-direction", &current_buffer->display_direction,
    "*Non-nil means lines in the buffer are displayed right to left.\n\
Nil means left to rigbt.\n\
Why don't you try to implement up to down?");
/* end of patch */
...
/* 87.12.9, 89.3.10, 91.10.21  by K.Handa */
  DEFVAR_PER_BUFFER ("mc-flag", &current_buffer->mc_flag,
    "non-nil means that this buffer recognizes multi-lingual code.");

  DEFVAR_PER_BUFFER ("file-coding-system", &current_buffer->file_coding_system,
    "Coding-system for file output.");

/* 89.11.21, 91.10.27, 92.1.30 by K.Handa, 92.7.14 by T.Ito */
  DEFVAR_PER_BUFFER ("attributed-region", &current_buffer->attributed_region,
    "Attributed region of current buffer.\n\
Should be a list of conses of MARKER_i and ATTR_i,\n\
where MARKER_i indicates starting point attribute ATTR_i.");
/* end of patch */

/* 91.10.16 by S.Hirano, 91.12.6, 92.11.1, 93.3.3 by M.Higashida */
#ifdef FILE_TRANSLATION_MODE
  DEFVAR_PER_BUFFER ("file-translation-mode",
		     &current_buffer->file_translation_mode,
    "*Translation mode for file input/output.");
#endif
#if (defined(MSDOS) && defined(FEPCTRL)) || (defined(WIN32) && defined(USE_IME))
  DEFVAR_PER_BUFFER ("fep-mode", &current_buffer->fep_mode, "");
#endif
/* end of patch */
