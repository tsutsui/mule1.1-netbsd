...
/* 92.1.16  modified for Mule Ver.0.9.0 by K.Handa <handa@etl.go.jp> */
...
Lisp_Object Vselect_window_hook; /* 92.2.17 by K.Handa */
  ...
  (Lisp_Object window)
{
  ...
  if (XFASTINT (w) + XFASTINT (decode_window (window)->left) == screen_width)
    return w;
  XSETINT (w, XFASTINT (w) - 1);
  return w;
}
...
NCOL should be zero or positive.")
  (register Lisp_Object window, register Lisp_Object ncol)
{
  register struct window *w;
...
not to count the minibuffer even if it is active.")
  (register Lisp_Object window, register Lisp_Object mini)
{
  register Lisp_Object tem;
  if (NILP (window))
    window = selected_window;
...
BUFFER can be a buffer or buffer name.")
  (register Lisp_Object window, register Lisp_Object buffer)
{
  register Lisp_Object tem;
  register struct window *w = decode_window (window);
  ...
  (register Lisp_Object window)
{
 ...
 select_window_exit:
  if (!EQ (Vselect_window_hook, Qnil)) /* 92.2.17 by K.Handa */
...
Returns the window displaying BUFFER.")
  (register Lisp_Object buffer, register Lisp_Object notthiswindow)
{
  register Lisp_Object window;
...
and put SIZE columns in the first of the pair.")
  (Lisp_Object window, Lisp_Object chsize, Lisp_Object horflag)
{
...
From program, optional second arg non-nil means grow sideways ARG columns.")
  (register Lisp_Object n, register Lisp_Object side)
{
  CHECK_NUMBER (n, 0);
...
void
change_window_height (register int delta, int widthflag)
{
...
void
scroll_command (register Lisp_Object n, int direction)
{
...
static int
save_window_save (Lisp_Object window, struct Lisp_Vector *vector, int i, int maxwindow)
{
  register struct saved_window *p;
  register struct window *w;
  register Lisp_Object tem;
...
Used by with-output-to-temp-buffer.");
  ...
  DEFVAR_LISP ("select-window-hook", &Vselect_window_hook,
    "Non-nil means call as function after selecting window.\n\
The hook is called with two argument OLD-WINDOW and NEW-WINDOW");
  Vselect_window_hook = Qnil;
