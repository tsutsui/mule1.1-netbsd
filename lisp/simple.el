...
;;; Basic editing Commands for Egg on Mule
;;; Modified by Satoru Tomura(tomura@etl.go.jp)
  
;;; 92.9.11  modified for Mule Ver.0.9.6 by K.Handa <handa@etl.go.jp>
;;;	Bug in do-auto-fill fixed.
;;; 92.10.18 modified for Mule Ver.0.9.6 by T.Enami <enami@sys.ptg.sony.co.jp>
;;;	kill-forward-chars and kill-backward-chars cope with mult-byte chars.
;;; 93.2.10  modified for Mule Ver.0.9.7.1 by N.Kamei <zic@tci.toshiba.co.jp>
;;;	In what-cursor-position, ctl-hexa is concerned.
;;; 93.8.13  modified for Mule Ver.1.1 by K.Nagahashi <nag@flab.fujitsu.co.jp>
;;;	count-lines is changed to that of Emacs 19.


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Change Log before Ver.0.9.6
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;		  what-cursor-position handles multi-byte characters.
;;; Mule Ver.0.9.3
;;;   Ver.18.57   modified by K.Handa 92-Mar-2
;;; Mule Ver.0.9.0
;;;;  Ver.18.55    created by S.Tomura 89-Nov-17
;;; Nemacs Ver.3.2
;;;;  Ver.18.52a   modified by S.Tomura 89-Oct-21
;;;;  Ver.18.52    created by S.Tomura 89-Mar-16
;;; Nemacs Ver.3.0
;;;;  Ver.18.50g   modified by S.Tomura 88-Nov-18
;;;;  Ver.18.50f   modified by S.Tomura 88-Aug-25
;;;;  Ver.18.50e   modified by S.Tomura 88-Jul-09
;;;;  Ver.18.50d   modified by S.Tomura 88-Jun-23
;;;;  Ver.18.50c   modified by S.Tomura 88-Jun-20
;;;;  Ver.18.50b   modified by S.Tomura 88-Jun-16
;;;;  Ver.18.50a   modified by S.Tomura 88-Jun-11
;;;;  Ver.18.50    created by S.Tomura 88-Jun-11
;;; Nemacs Ver.2.1  
;;;;  Ver.18.47a2  modified by S.Tomura 88-Jun-2
;;;;  Ver.18.47a1  modified by S.Tomura 88-Feb-9
;;;;  Ver.18.47a   modified by S.Tomura 88-Feb-8
;;;;               Kanji auto-fill added.
...
(defun kill-forward-chars (arg)
  ...
  (kill-region (point)			;92.10.18 by T.Enami
	       (save-excursion (forward-char arg) (point))))

(defun kill-backward-chars (arg)
  (if (listp arg) (setq arg (car arg)))
  (if (eq arg '-) (setq arg -1))
  (kill-region (point)			;92.10.18 by T.Enami
	       (save-excursion (backward-char arg) (point))))
...
;; 93.8.13 by K.Nagahashi
;; New definition to be compatible with goto-lines.
;; Copied from Emacs 19.
(defun count-lines (start end)
...
(defun what-cursor-position ()
  ...
  (let* ((char (following-char))
	 ...
	 ;; 92.4.15 by T.Enami, 93.2.10 by N.Kamei
         (char-desc (format (if ctl-hexa "Char: %s (0x%x) " "Char: %s (0%o) ")
                            (if (eq (char-leading-char char) 0)
                                (single-key-description char)
                              (text-char-description char))
                            char))
         (charset-doc (if (null mc-flag)
                          ""
                        (charset-doc (char-leading-char char))))
         (range (if (or (/= beg 1) (/= end (1+ total)))
                    (format " <%d - %d>" beg end)
                  ""))
	 ;; end of patch
	 (col (current-column)))
    (if (= pos end)			;; 92.4.15 by T.Enami
        (message "point=%d of %d(%d%%)%s column %d %s"
                 pos total percent range col hscroll)
      (message "%s point=%d of %d(%d%%)%s column %d %s %s"
               char-desc pos total percent range col hscroll charset-doc))))
...
(defun transpose-subr-1 ()
  ...
  (let ((word1 (buffer-substring start1 end1))
    ...
    (delete-region (point) (+ (point) (length word1))) ; 91.12.1 by K.Handa
    (insert word2)))

  ...
  (let ((fill-point
	 (let ((opoint (point))
	   ...
	   (save-excursion
	     ...
	     ;;; find the point where there is no spaces before and
	     ;;; the preceding-char is kanji or the following-char is space.
	     (let ((p (point)) ch)
    ...
    (if (save-excursion
	  ...
	  (not
	   ;;; patch by S. Tomura 88-Jun-23
	   ;;;日本語の場合、禁則処理で分割点が行末にくることがある。
	   ;;;(bolp)
	   (or (bolp) (eolp))
           ;;; end of patch 
	   ))
...
The body of the continued comment is indented under the previous comment line."
  ...
  (let (comcol comstart)
    ...
    (kinsoku-shori)
    ;;; end of patch
    ;;; patched by S.Tomura 88-Jun-20
    ;;;(delete-region (point)
    ;;;		   (progn (skip-chars-forward " \t")
    ;;;			  (point)))
    ;;; 全角文字の前後の半角空白１文字は保存する。
    (if (not mc-flag) 
	(delete-region (point)
		       (progn (skip-chars-forward " \t")
			      (point)))
	...
	;; patched by K.Handa 92-Mar-2
	(forward-char -1)
	(if (and (< start end) (looking-at word-across-newline))
	    (progn
	      (forward-char 1)
	      (insert ?  ))
	  (forward-char 1)
  ...
  (and (> (point) (1+ (point-min)))
	 ...
	 (save-excursion
	   ...
	   (and blinkpos (/= (char-syntax (char-after blinkpos))
			     ?\$)
		(setq mismatch
		      (/= (char-after (1- oldpos));; 91.1.10 patch by K.Handa
			  (char-syntax-match (char-after blinkpos))
			  ;;(char-after (1- oldpos))
