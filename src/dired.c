...

/* 91.10.27, 92.11.27, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
#include <sys/types.h>
#include <time.h>

DEFUN ("time-to-string", Ftime_to_string, Stime_to_string,
       1, 2, 0, "")
  (Lisp_Object cons_time, Lisp_Object trancate)
{
  Lisp_Object high, low;
  long t, current_time;
  char tem[40];
  
  CHECK_CONS (cons_time, 0);
  high = Fcar (cons_time);
  low  = Fcar (Fcdr (cons_time));
  CHECK_NUMBER (high, 0);
  CHECK_NUMBER (low, 0);
  t = (XINT (high) << 16) | XINT (low);
  ...
  if (!NILP (trancate))
    {
      current_time = time (0);
      if (current_time > t + 6L * 30L * 24L * 60L * 60L /* Old. */
	  || current_time < t - 60L * 60L) /* In the future. */
	{
...
/* 91.10.27, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
  defsubr (&Stime_to_string);
#endif
/* end of patch */
