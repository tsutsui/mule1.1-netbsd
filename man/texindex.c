...
/* 89.11.23 modified for Nemacs Ver.3.2 by K.Handa
   to handle Japanese code (EUC).  */
...
/* patch by K.Handa  89.11.23 */
#define EUC_P(c) ((unsigned char)c >= 0xa1 && (unsigned char)c <= 0xfe)
/* end of patch */
...
/* patch by K.Handa  89.11.23 */
  for (i = 0xa1; i <= 0xfe; i++)
    char_order[i] = 512 + i;
  char_order[0xa4] = char_order[0xa5];
/* end of patch */
}
     ...
     long pos2;
{
  ...
  if (keyfield->numeric)
    ...
    {
/* patch by K.Handa  89.11.23 */
/* original types are 'char' */
      unsigned char *p1 = (unsigned char *)start1;
      unsigned char *p2 = (unsigned char *)start2;
      unsigned char *e1 = (unsigned char *)start1 + length1;
      unsigned char *e2 = (unsigned char *)start2 + length2;
/* end of patch */
...
/* patch by K.Handa  89.11.23 */
	  if (EUC_P(c1) && c2) {
	    ...
	    c2 = *p2++;
	    if (*(p1-2) == 0xa3) {
	      if (0xe1 <= c1 && c1 <= 0xfa) c1 -= 0x20;
	      if (0xe1 <= c2 && c2 <= 0xfa) c2 -= 0x20;
...
/* end of patch */
      ...
      /* ... and for Hiragana-Katakana. */ /* patch by K.Handa  89.11.23 */
/* type casting patch by K.Handa  89.11.23 */
      p1 = (unsigned char *)start1;
      p2 = (unsigned char *)start2;
/* end of patch */
      while (1)
	{
	  int c1, c2;
...
init_index ()
{
  ...
  lastinitial1[0] = 0;
  lastinitial1[1] = 0;
  lastinitial1[2] = 0;		/* patch by K.Handa  89.11.23 */
  lastinitiallength = 0;
...
/* patch by K.Handa  89.11.23 */
	...
	initial1[2] = 0;
	initiallength = 2;
	if ((unsigned char)*p == 0xa4) initial1[0] = (char)0xa5;
	else if ((unsigned char)*p == 0xa3
		 && (unsigned char)initial1[1] >= 0xe1
		 && (unsigned char)initial1[1] <= 0xfa)
	  initial1[1] -= 0x20;
      } else {
/* end of patch */
      initial1[1] = 0;
	  ...
	  if (initial == initial1)
	    {
	      ...
	      *lastinitial1 = *initial1;
	      lastinitial1[1] = initial1[1]; /* patch by K.Handa  89.11.23 */
	    }
	  else
	    {
	      lastinitial = initial;
