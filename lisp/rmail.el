...
;;;	- In rmail-expunge, forward-char at the tail is changed to goto-char.
;;;	- In rmail-reply, "Re:" in subject is handled correctly.
...
Assumes that the visible text of the message is not changed by FUNCTION."
  (save-excursion
    (let ((obeg (- (point-max) (point-min)))) ;94.2.23 by T.Saneto
      (unwind-protect
	  (progn
...
(defun rmail-expunge ()
  ...
  (let* ((omax (- (buffer-size) (point-max)))
	...
	(save-excursion
      ...
      ;; 92.3.26 by M.Kuwada - (forward-char opoint) is changed.
