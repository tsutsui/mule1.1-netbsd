...
;;; 92.3.26  modified for Mule Ver.0.9.2
;;;	by T.Nakagawa <nakagawa@is.titech.ac.jp>
;;;	In hack-local-variables, variable prefixlen is removed.
;;; 92.9.14  modified for Mule Ver.0.9.6 by Y.Kawabe <kawabe@sra.co.jp>
;;;	The coding-system of auto-saved file is *internal*.
;;; 92.10.16 modified for Mule Ver.0.9.6 by K.Handa <handa@etl.go.jp>
;;;	backup-buffer uses variable 'backup-filename'.
;;;	list-directory uses variable 'shell-command-option'.
...
for current buffer."
  ...
  (save-excursion
	...
	(let ((continue t)
	      prefix suffix beg)	; 92.3.26 by T.Nakagawa
	  ...
	  (if prefix (setq		; 92.3.26 by T.Nakagawa
	  ...
	  (while continue
		...
		(if (looking-at prefix)
		    (goto-char (match-end 0)) ; 92.3.26 by T.Nakagawa
...
;	      (error "Cannot save buffer in directory %s" buffer-file-name))
	  (condition-case ()
	    ...
	    (file-error
	     ...
	     ;; 92.10.16 by K.Handa
	     (setq backupname (expand-file-name backup-filename))
  ...
  (let ((file-name (let ((buffer-file-name file))
    ...
    (cond ((not (file-newer-than-file-p file-name file))
	  ...
	  ((save-window-excursion
	   ...
	   (let ((buffer-read-only nil))
	     (erase-buffer)
	     (si:insert-file-contents file-name nil)) ; 92.9.14 by Y.Kawabe
	   (after-find-file nil))
  ...
  (let ((switches (if verbose list-directory-verbose-switches
	...
	(let ((default-directory (file-name-directory dirname)))
	  (call-process shell-file-name nil standard-output nil
			shell-command-option (concat "ls " ;92.10.16 by K.Handa
				     switches " "
