...
/* 91.10.16 by S.Hirano <hirano@tkl.iis.u-tokyo.ac.jp> for go32 1.05 */
/* 92.10.16 by M.Higashida <manabu@sigmath.osaka-u.ac.jp> for go32 1.08 */
#if defined(MSDOS) && defined(GO32)
#if 0
/*
 * DJGCC startup for 1.08 (have no backward compatibility!)
 * Called as start (argc, argv, envp)
 */
asm ("	.text");
asm ("	.globl	__tstart");  /* this label is used in start_of_text()/sysdep.c */
asm ("__tstart:	");
asm ("  .long 0");	/* for null pointer access */
asm ("  .long 0");
asm ("	.globl	__start");
asm ("__start:	");
asm ("	movl	%esi,___pid");
asm ("	movl	%edi,___transfer_buffer");
asm ("	movl	%ebx,_ScreenPrimary");
asm ("	movl	%ebp,_ScreenSecondary");
asm ("	xorl	%esi,%esi");
asm ("	xorl	%edi,%edi");
asm ("	xorl	%ebp,%ebp");
asm ("	xorl	%ebx,%ebx");
...
asm ("	.long	0");
/**/
asm ("	.globl	___transfer_buffer");
asm ("___transfer_buffer:");
asm ("	.long	0");
/**/
asm ("	.globl	_ScreenPrimary");
asm ("_ScreenPrimary:");
asm ("	.long	0");
/**/
asm ("	.globl	_ScreenSecondary");
asm ("_ScreenSecondary:");
asm ("	.long	0");
/* end of startup */
#else
/*
 * DJGCC startup for 1.10 (have no backward compatibility!)
 * Called as start (argc, argv, envp)
 */
asm ("	.text");
asm ("	.globl	__tstart");  /* this label is used in start_of_text()/sysdep.c */
...
asm ("	.long	0");
/**/
asm ("	.globl	__hard_master");
asm ("	.globl	__hard_slave");
asm ("	.globl	__core_select");
asm ("__hard_master:");
asm ("	.byte	0");
asm ("__hard_slave:");
asm ("	.byte	0");
...
/* end of startup */
#endif
#endif /* MSDOS and GO32 */
/* end of patch */


#ifdef    luna88k
/*
   For luna88k running Mach.
   sp(r31) points argc.
   Note: current version of gcc dumps core when compiling this file
   specifying -g and -O together.
...
start1(zargc)
    int zargc;
{
    register int *xargc = &zargc;	/* magic!! force r2 to write back */
    register int argc;
    register char **argv;
