...
;;; 91.3.23  modified for Nemacs Ver.3.3.3 by S.Tomura
;;; 91.11.18 modified for Nemacs Ver.4.0.0 by K.Handa <handa@etl.go.jp>
;;; 92.3.5   modified for Mule Ver.0.9.0 by K.Handa <handa@etl.go.jp>
;;; 93.6.4   modified for Mule Ver.0.9.8 by K.Handa <handa@etl.go.jp>
;;;	'isearch-read-char' is introduced for the future facility.
...
;;; patch by K.Handa 88.5.28, S.Tomura 91.3.23, K.Handa 92.3.5
(defvar search-string-char ?\C-k
  "Key to invoke read-string function within incremental search.
Initialy '\C-k' is binded.")

(defvar search-string-char-prompt "Enter string... ")

(defvar current-isearch-message ""
  "A string shown in echo area while incremental searching.")

(defvar isearch-read-string-hook nil
  "A hook function called within incremental search when you type a key
binded to the variable search-string-char.
See also current-isearch-message (variable)")
;;; end of patch

;; 93.6.4 by K.Handa
(defvar isearch-read-char 'read-char
  "read-char function used in interactive search.")
  ...
  (let ((search-string "")
    ...
    (save-window-excursion
	 ...
	 (let ((char (if quit-flag
			 ?\C-g
		       (funcall isearch-read-char)))) ;93.6.4 by K.Handa
	   (setq quit-flag nil adjusted nil)
	   ;; Meta character means exit search.
	   (cond ((and (numberp char)	;93.6.4 by K.Handa
		       (>= char 128) (< char 256)
		       search-exit-option)
		 ...
		 ((or (eq char search-repeat-char)
		  ...
		  (if (eq forward (eq char search-repeat-char))
			  ...
			  (setq search-string
				...
				(mapconcat 'text-char-description
					   (string-to-char-list search-string)
					   ""))
		  ...
		  (or (equal search-string "")
		      (isearch-search))
		  (isearch-push-state))
		 ((eq char search-delete-char) ;93.6.4 by K.Handa
				 ...
				 search-message
				   (concat search-message
					   ...
					   (mapconcat
					    ...
					    (string-to-char-list word)
					    "")))))
 			;;; patch by K.Handa 88.5.27, S.Tomura 91.3.23
			;;; patch by K.Handa 92.3.5
			((eq char search-string-char)
			 (let ((str
				(if isearch-read-string-hook
				    (funcall isearch-read-string-hook)
				  (read-string
				   (concat search-string-char-prompt
					   current-isearch-message)))))
			   (setq search-string (concat search-string str)
				 search-message (concat search-message str))))
			;; 93.6.4 by K.Handa
			((stringp char)
			 (setq search-string (concat search-string char)
			       search-message (concat search-message char)))
			;;; end of patch
