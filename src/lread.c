...
/* 89.10.24 modified for Nemacs Ver.3.2 by K.Handa */
/* 90.1.26  modified for Nemacs Ver.3.3.1 by K.Handa
		close 'fd' after load-hook */
/* 90.6.6   modified for Nemacs Ver.3.3.2 by abe@sunrise.hallab.co.jp
	'GCPRO2(str, f)' in Fload() */
/* 91.10.30 modified for Nemacs Ver.4.0.0 by K.Handa <handa@etl.go.jp> */
...
/* 92.12.22 modified for Mule Ver.0.9.7 by K.Handa <handa@etl.go.jp>
		by M.Higashida <manabu@sigmath.osaka-u.ac.jp>
	In Floadablep(), Fsubstitute_in_file_name() is called. */
/* 93.2.12  modified for Mule Ver.0.9.7.1 by K.Handa <handa@etl.go.jp>
	In read1() and read_escape(), private chars and composite chars
...
/* 91.10.16 by S.Hirano */
    ...
    {
	int c;	
	c = getc (instream); /* WHY? DJGCC is buggy? */
	return c;
    }
#else
...

/* 91.10.30 by K.Handa */
DEFUN ("loadablep", Floadablep, Sloadablep, 1, 2, 0,
  "Search for a file whose name is STR, looking in directories list\n\
load-path, trying suffixes from .elc or .el.\n\
If optional second arg NOSUFFIX is non-nil, don't try adding\n\
 the suffixes.\n\
On success, returns a file name string, or else returns NIL.")
  (Lisp_Object str, Lisp_Object nosuffix)
{
  Lisp_Object fn;
  int fd;

  CHECK_STRING (str, 0);
  str = Fsubstitute_in_file_name (str);	/* 92.12.22 by M.Higashida */
  fd = openp(Vload_path, str, NILP (nosuffix) ? ".elc:.el:" : "", &fn, 0);
  if (fd > 0) close(fd);
  return fn;
}
/* end of patch */
...
#ifdef nec_ews_svr2				       /* hir@nec, 1992.8.21 */
	  {   /* if file name exceeds 14 chars, cut it */
	      char *p = strrchr(fn,'/');
	      if (p && strlen(p) > 15) *(p+15) = '\0';
	  }
#endif /* nec_ews_svr2 */
...
read1 (register Lisp_Object readcharfun)
{
 ...
 retry:
  ...
  switch (c)
	...
	else if (LC_P(c)) /* 91.11.13 by K.Handa */
	  {
	    unsigned char str[5]; /* 93.3.10 by K.Handa */
	    register int i;

	    str[0] = c;
	    i = 1;
	    while ((c = READCHAR) >= 0 && NONASCII_P (c))
	      if (i < 5) str[i++] = c;
	    UNREAD (c);
	    c = STRtoCHAR (str, i);
	  }
...
syms_of_read (void)
{
  ...
  defsubr (&Sload);
  defsubr (&Sloadablep);	/* 91.10.30 by K.Handa */
