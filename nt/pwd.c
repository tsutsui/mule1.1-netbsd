...
/* 92.11.14  modified for Mule Ver.0.9.6
...
{
  "LOGIN", "USER", "USERNAME", "LOGNAME", "MAILNAME", (char *) 0
};

static char *gecos_strings[] =
{
  "FULLNAME", "USERFULLNAME", (char *) 0
};
...
static char *unknown = "unknown";	/* if all else fails ... */
...
getgecos ()
{
  if (!gecos)			/* have we been called before? */
    gecos = lookup_env (gecos_strings);

  if (!gecos)			/* have we been successful? */
    gecos = unknown;
