...
static void
printchar (
    unsigned int ch,		/* 91.11.17 by K.Handa */
    Lisp_Object fun
)
{
  Lisp_Object ch1;
  ...
  if (EQ (fun, Qnil))
    {
      QUIT;
      Finsert_char (ch, 1);	/* 91.11.17 by K.Handa */
      return;
    }
  if (EQ (fun, Qt))
    {
      ...
      if (echo_area_contents != message_buf || !message_buf_print)
	{
      ...
      if (printbufidx < MSG_BUF_SIZE) {	/* 91.11.17, 92.4.15 by K.Handa */
	unsigned char str[4], *p = str;
	int i = CHARtoSTR (ch, str);

	if (i <= MSG_BUF_SIZE - printbufidx)
	  while (i--) message_buf[printbufidx++] = *p++;
      }
...
strout (char *ptr, int size, Lisp_Object printcharfun)
{
  ...
  if (EQ (printcharfun, Qt))
      ...
      if (echo_area_contents != message_buf || !message_buf_print)
	{
      ...
      if (i > MSG_BUF_SIZE - printbufidx) /* 92.4.15 by K.Handa */
	i = MSG_BUF_SIZE - printbufidx;
  ...
  (Lisp_Object ch, Lisp_Object printcharfun)
{
  ...
  CHECK_CHARACTER (ch, 0);	/* 91.11.17 by K.Handa */
  PRINTPREPARE;
  PRINTCHAR (XFASTINT (ch));	/* 91.11.17 by K.Handa */
  PRINTFINISH;
  return ch;
}
