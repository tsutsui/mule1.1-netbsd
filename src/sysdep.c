...
/* 92.4.30  modified for Mule Ver.0.9.4 by K.Handa <handa@etl.go.jp>
	The same modification above is applied for Mule. */
/* 92.9.18 modified for Mule Ver.0.9.6 by R.Sladkey <jrs@world.std.com>
	Patch for linux 0.96c pl2.
	adds a new ifdef on HAVE_SYS_SIGLIST becuase most
	USG systems don't have one but Linux does and the signal
	numbers are different than usual. */
/* 92.10.1 modified for Mule Ver.0.9.6 by Y.Akiba <akiba@cbs.canon.co.jp>
	child_setup_tty() is modified for NeXT. */
...
/* 93.6.15  modified for Mule Ver.0.9.8 by T.Enami <enami@sys.ptg.sony.co.jp>
	Some systems don't have (long)clock(). */
/* 93.7.8   modified for Mule Ver.0.9.8 by
   				N.Takahashi <ntakahas@etlnao.etl.go.jp>
	For HPUX, HAVE_RANDOM is commented out. */
/* 93.7.8   modified for Mule Ver.0.9.8 by M.Masuya <masu@bi.a.u-tokyo.ac.jp>
	Only for HPUX9, HAVE_RANDOM is commented out. */
/* 93.7.19  modified for Mule Ver.0.9.8 by K.Handa <handa@etl.go.jp>
	#ifdef MISSING_CLOCK -> #ifndef HAVE_CLOCK. */
/* 93.7.26  modified for Mule Ver.0.9.8 by K.Handa <handa@etl.go.jp>
	Define HAVE_CLOCK for USG and VMS. */
...
/* 91.10.20, 93.2.25 by M.Higashida */
...
#ifndef HAVE_TERMIO
#ifndef VMS
/* 91.10.16 by S.Hirano, 92.11.4, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
#ifdef EMX
#include <sys/ioctl.h>
...
discard_tty_input (void)
{
  ...
  queue_kbd_input ();
#else /* not VMS */
/* 91.10.16 by S.Hirano, 92.11.3, 93.2.17 by M.Higashida */
#ifdef WIN32
  /* do something! */
#else /* not WIN32 */
#ifdef MSDOS
...
/* 91.10.16 by S.Hirano, 92.11.4, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
      /* OSPEED () returns 15; 38400 baud since we use console */
#else /* not MSDOS nor WIN32 */
      SETOSPEED (sg, B9600);
      tcgetattr (0, &sg);
#endif /* not MSDOS nor WIN32 */
...
/* AIX enhanced edit loses NULs, so disable it */
  ...
  s.sg_ospeed = B9600;
  {
    ...
    lmode |= (LPASS8 | LPASS8OUT | LPENDIN | LDECCTQ);
    ioctl(out,TIOCLSET,&lmode);
  }
#else
/* 90.1.31, 92.4.30 by K.Handa */
#ifdef LPASS8  /*  Effective, at least, on SunOS 3.?, 4.?, and 4.3bsd */
  {
    int arg=LPASS8;
    ioctl (out, TIOCLBIS, &arg);
  }
#endif /* LPASS8 */
#endif /* NeXT */
#endif /* not HAVE_TERMIO */
...
sys_suspend (void)
{
  ...
  return -1;
#else
/* 92.10.4, 92.11.3, 93.2.17 by M.Higashida */
#ifdef MSDOS
#undef SIGTSTP
#endif
/* end of patch */
...
/* 91.10.20, 92.11.14, 93.2.17 by M.Higashida */
#ifndef MSDOS
...
#endif
/* 91.10.20, 92.11.14 by M.Higashida */
#ifdef MSDOS
      system (sh);
      chdir (oldwd);
#else /* not MSDOS */
...
#else /* no FASYNC */
#if defined(STRIDE) || defined(linux)		/* Stride doesn't have FASYNC - use FIOASYNC */
...
#if defined(nec_ews) && defined(CANNA)		/* hir@nec, 1992.2.19 */
#include <jtermio.h>
static struct jtermio old_gttj,jsg;
#endif /* nec_ews */
...
#endif
#else /* not VMS */
/* 91.10.16 S.Hirano, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
  /* do nothing */
#else /* not MSDOS nor WIN32 */
...
#else
/* 91.10.20, 92.11.3 by M.Higashida */
#ifdef MSDOS
#ifdef GO32
      tty_raw ();
#else
...
#undef _IOFBF	/* DJGCC's setvbuf() is buggy */
#endif
/* end of patch */
#ifdef _IOFBF
...
#ifdef  nec_ews_svr2			/* hir@nec,1989.2.2 */
...
#undef TIOCGWINSZ
#endif
#define TIOCGWINSZ TCGSIZE
#else /* not nec_ews_svr2 */
#ifdef TIOCGSIZE
#ifndef TIOCGWINSZ
...
/* 91.10.20, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
  {
    ...
    if (xwidth < 0)
      if ((e = egetenv ("COLUMNS")) == 0)
	*widthp = xwidth = 0;
      else 
	*widthp = xwidth = atoi (e);
    else
      *widthp = xwidth;

    if (xheight < 0)
      if ((e = egetenv ("ROWS")) == 0 && (e = egetenv ("LINES")) == 0)
	*heightp = xheight = 0;
      else 
	*heightp = xheight = atoi (e);
    else
      *heightp = xheight;
  }
#else /* not MSDOS nor WIN32 */
...
reset_sys_modes (void)
{
  ...
  SYS$QIOW (0, input_chan, IO$_SETMODE, &input_iosb, 0, 0,
	    &old_gtty.class, 12, 0, 0, 0, 0);
#else /* not VMS */
/* 91.10.20, 92.11.3, 93.2.17 by M.Higashida */
#if defined(MSDOS) || defined(WIN32)
#ifdef GO32
  tty_reset ();
#endif
...
#if defined(nec_ews) && defined(CANNA)		/* hir@nec, 1992.2.19 */
  while (ioctl (0, TCJSETA, &old_gttj) < 0 && errno == EINTR);	
#endif /* nec_ews */
...
#endif /* BSD4_1 or WIN32 */

#ifdef HPUX
#ifdef X11
#ifndef hpux9			/* 93.7.8 by M.Masuya */
#define HAVE_RANDOM		/* 93.7.8 by N.Takahashi */
#endif
#endif
#endif
...
void
sys_fwrite (void *ptr, unsigned size, unsigned num, FILE *fp)
{
...
#endif /* AIX */

/* 91.10.16 by S.Hirano, 91.10.20, 93.2.17 M.Higashida */
#if defined(MSDOS) || defined(WIN32)
void
input_wait_timeout (
    int time_to_wait		/* Time to wait, in seconds */
)
{
  time_t tnow, tthen;
...
void
unrequest_sigio (void) { croak ("unrequest_sigio"); }
void
request_sigio (void) { croak ("request_sigio"); }

#undef gethostname
...
{
  char *q;
#ifdef WIN32 /* 93.3.3 by M.Higashida */
  char hostname[MAX_COMPUTERNAME_LENGTH+1];
  DWORD namelen = MAX_COMPUTERNAME_LENGTH;

  if (GetComputerName (hostname, &namelen))
    strcpy (p, hostname);
  else
#endif
...
int
sleep (int a) { return _sleep (a * 1000); }
int
kill (int a, int b) { return -1; }
sigsetmask (mask) int mask; { return 0; }
int
getpid (void) { return _getpid (); }
int
getdisk (void) { return (_getdrive () - 1); }
int
setdisk (int a) { return _chdrive (a + 1); }
#endif /* WIN32 */
/* end of patch */
...
#define TIMEVAL_TO_MICROSEC(tvp) ((tvp)->tv_sec * 1000000 + (tvp)->tv_usec)
#define RUSAGE_TOTAL(rp) \
  (TIMEVAL_TO_MICROSEC (&(rp)->ru_utime) + \
   TIMEVAL_TO_MICROSEC (&(rp)->ru_stime))
