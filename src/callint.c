...
/* 92.4.8   modified for Mule Ver.0.9.3 by T.Enami <enami@sys.ptg.sony.co.jp>
	In Fcall_interactively(), 'string' should be safe from gc. */
/* 92.12.21 modified for Mule Ver.0.9.7 by K.Handa <handa@etl.go.jp>
	In Fcall_interactively(), coding-system reading is supported. */
...
/* 92.12.21 by K.Handa */
	case 'i':		/* Ignore an argument -- Does no do I/O */
	  varies[i] = -1;
	  break;
/* end of patch */
...
/* 92.12.21 by K.Handa */
	case 'Z':		/* Coding-system symbol or nil if no prefix */
	  if (NULL (prefix_arg)) {
	    ...
	    visargs[i] = Fread_non_nil_coding_system (build_string (prompt));
	    teml = visargs[i];
	    args[i] = Fintern (teml, Qnil);
	  }
	  break;
	case 'z':		/* Coding-system symbol */
	  visargs[i] = Fread_coding_system (build_string (prompt));
	  teml = visargs[i];
	  args[i] = Fintern (teml, Qnil);
	  break;
/* end of patch */
